  ...
  try {
    ...
    lchref = lchref.setQueryValue("NamedFilter", thisForm.namedFilter.value);
      ...
      for (var i = 0; i < thisForm.namedFilter.length; i++) {
        /* if multiple and all selected then add none which is all */
        if($(thisForm.namedFilter[i]).find('option:selected').length==$(thisForm.namedFilter[i]).find('option').length
           && thisForm.namedFilter[i].multiple == true)
           lcNamedFilter = lcNamedFilter + ',';
        else
          lcNamedFilter = lcNamedFilter + "," + $(thisForm.namedFilter[i]).val().toString().replaceAll(',',';');
      }
...
function selectRow(phObject,pcExt) {
  ...
  if (bCtrlKey||bAltKey||pcExt.includes("C")) {
  ...
  else if (bShiftKey||pcExt.includes("S")) {
...
function catsdata(phItem) {
  var lcURL = "wouajax.p"
  + location.search
  + "&ajaxmethod=syasur.catsdata";
  $.ajax({
    type: "post",
    url: lcURL,
    data: {"query":phItem.getAttribute('data-query'),
           "table":phItem.getAttribute('data-table'),
           "mainarea":cMainArea},
    error: function(pcText) {
    console.log(pcText);
    alert('AJAX Failure');
        ...
        /* this is the TD */
         ...
         /* col==1 is the open/detail column unless suppressed */
           ...
           if(pbFreeze) {
             ...
             /* and can be in <NOBR> if decimal */
             var lhCell = $(this).find('div.freezetd')[0];
             if(phData==null) {
               lhCell.innerHTML=cBodyUpdateUndo[liCol-1];
             }
             else {
               ...
               if(lbSet){
                 if(phData=="") {
                   ...
                   lhCell.innerHTML=phData.row.col[liCol-1];
                 }
               }
             }
           }
