...
function setReturnField(pcField, pcSource) {
  ...
  try {
    ...
    if (lhField==undefined) {
      setReturnData(pcField,pcSource);
      return;
    }
        ...
        */
        ...
        /* thisForm not included on hide/show so we can cover non-updateable
         ...
         disableThisField(lcField,getReturn(lcSource)=="yes","hide");
          ...
          var lhField = eval("thisForm." + lcField);
          
        	if (lhField!=undefined) {
            if (lbEnableOrDisable) disableThisField
             ("thisForm." + lcField,getReturn(lcSource)=="yes","disable");
            else {
            	setReturnField(lcField,lcSource);

              // Field may be hidden, but have a visible counterpart with the
            	// same ID.
              try {
                if (lhField.type=="hidden") {
                	var lcValue = lhField.value;
                  lhField = lhField.parentNode;
                  if (lhField.id==lcField) lhField.innerHTML =
                   lhField.innerHTML.split(">")[0].concat(">",lcValue);
                }
              }
              catch(e) {}
            }
