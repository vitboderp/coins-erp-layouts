    ...
    if (lcFrameName==lcFrameID||lcFrameName.replace("%","$A$")==lcFrameID) {
      if ($iframe[i].contentDocument.location.href=="about:blank") {
        if ($iframe[i].name.indexOf("!|")==-1) {
          ...
          var liAdjustment = parseInt(lcHref.getQueryValue('inlineHeightAdjustment'));
          if (!isNaN(liAdjustment)) {liHeight = liHeight - liAdjustment;}
...
function showWin(newnn,oldnn)
{
  ...
  { 
    if (oldnn!=-1) {
      ...
      $(oldObj).find('iframe.inlineframe').removeClass('active');
    }
    var newObj = document.getElementById(arrFrames[newnn]);
    newObj.style.display = '';
    $(newObj).find('iframe.inlineframe').addClass('active');
