...
(function() {
  ...
  $.widget("ui.dialogExtend", {
    ...
    _create: function() {
      ...
      var lhButtonBar=this.element.parent()
        .find('div.ui-dialog-titlebar-buttonpane');
     
     lhButtonBar.attr('title','');
     lhButtonBar.find('a.ui-dialog-titlebar-maximize').attr('title',cMaximize);
     lhButtonBar.find('a.ui-dialog-titlebar-minimize').attr('title',cMinimize);
     lhButtonBar.find('a.ui-dialog-titlebar-close').attr('title',cClose);
     lhButtonBar.find('a.ui-dialog-titlebar-restore').attr('title',cRestore);
    ...
    _setState: function(state) {
      ...
      /* set state in iframe */
      $(this.element[0]).find('iframe').each(function() {
        this.contentWindow.cFrameState=state;
        if(state!='minimized' 
          && this.contentWindow.mainarea
          && this.contentWindow.mainarea.getFrame
          && this.contentWindow.mainarea.getFrame.bDocumentRefresh)
          this.contentWindow.mainarea.getFrame.location.replace
            (this.contentWindow.mainarea.getFrame.location.href);
      });
    ...
    _initButtons: function() {
      ...
      buttonPane.css({
        ...
        "margin-top": "-10px",
        /*"height": "18px",*/
        "background-color": titlebar.css("background-color")
      });
      ...
      buttonPane.append('<a class="ui-dialog-titlebar-restore ui-corner-all ui-state-default" href="#"><span class="ui-icon ' + this.options.icons.restore + '" title="' + cRestore + '">restore</span></a>')
  .find('.ui-dialog-titlebar-restore').attr("role", "button").mouseover(function() {
      ...
      titlebar.attr('title',
         titlebar.find('span.ui-dialog-title').html());
      
      for (name in this.options.buttonbar) {

      /* add toolbar */
      buttonPane.append('<a class="ui-dialog-titlebar-'
        + name + ' ui-dialog-titlebar-hidemin'
        + ' ui-corner-all ui-state-default" href="#">'
        + '<span class="ui-icon ui-icon-' + name + '"'
        + ' title="' + this.options.buttonbar[name].title + '">'
        + this.options.buttonbar[name].title + '</span></a>')
     .find('.ui-dialog-titlebar-' + name)
     .attr("role", "button").mouseover(function() {
        return $(this).addClass("ui-state-hover");
      }).mouseout(function() {
        return $(this).removeClass("ui-state-hover");
      }).focus(function() {
      ...
      .click(bind(this.options.buttonbar[name],function(e) {
        e.preventDefault();
        this.action(e.currentTarget,e);
      })).end();
    ...
    _restore: function() {
      if (this._state !== "normal") {
        var liMaxZ=maxZ('div.ui-dialog');
        this["_restore_" + this._state]();
        this._setState("normal");
        var lh= $(this.element[0]).dialog("widget").focus();
        if(parseInt(lh.css('zIndex')) <= liMaxZ) 
          lh.css('zIndex',(liMaxZ + 1));
        var liMaxZ=maxZ('div.ui-dialog');
        return lh;
      }
    },
  ...
  $.extend(true, $.ui.dialogExtend.prototype, {
    ...
    minimize: function() {
      ...
      if ($("#dialog-extend-fixed-container").length) {
        ...
        fixedContainer = $('<div class="show" id="dialog-extend-fixed-container" onmouseover="statusOver()" onmouseout="statusOut()"></div>').appendTo("body");
/*
        fixedContainer.css({
          "position": "fixed",
          "bottom": 1,
      ...
      /* remember for restore animation */
      this.element[0].setAttribute('data-width',this.element[0].offsetWidth);
      this.element[0].setAttribute('data-height',this.element[0].offsetHeight);
      this.element[0].setAttribute('data-top',
                                   $(this.element[0]).offset().top);
      this.element[0].setAttribute('data-left',
                                   $(this.element[0]).offset().left);
      ...
      animateMinimize(this.element[0]);
    ...
    _minimize_restoreOnClose: function() {
      /* switch off hide animation */
      $(this).dialog("option","hide",false);
...
function animateMinimize(phDialog) {
  var lhDialog=$(phDialog);
  var lhAnimate=lhDialog.clone().empty().appendTo('body')
    .css({position:"absolute",
          left:lhDialog.offset().left,
          top:lhDialog.offset().top,
          width:lhDialog.width(),
          height:lhDialog.height(),
          zIndex:9999
        });
  lhAnimate.animate({left:($(window).width() / 2),top:0,
...
function animateRestore(phDialog,fnComplete) {
  var lhDialog=$(phDialog);
  if(lhDialog.dialog('option','hide')==false) return;
  ...
  var liLeft=parseInt(phDialog.getAttribute('data-left'));
  var liTop=parseInt(phDialog.getAttribute('data-top'));
  var liHeight=parseInt(phDialog.getAttribute('data-height'));
  var liWidth=parseInt(phDialog.getAttribute('data-width'));
